...
def demo(myaqua):
    if(not myaqua.add_animal("roi", 4, 10, 10, 1, 0 , "sc")):
        print("the spot is taken and the scalar wont be added")
    if(not myaqua.add_animal("ariel", 12, 30, 12, 0, 1, 'mo')):
        print("the spot is taken and the moly wont be added")
    if(not myaqua.add_animal("rotem", 3, 20, myaqua.aqua_height, 1, 0, 'sh')):
        print("the spot is taken and the shrimp wont be added")
    if (not myaqua.add_animal("liel", 13, 30, myaqua.aqua_height, 0, 0, 'oc')):
        print("the spot is taken and the ocypode wont be added")
    myaqua.feed_all()
    for i in range (120):
        myaqua.next_turn()
        for i in myaqua.anim:
            myaqua.print_animal_on_board(i)
        myaqua.print_board()
        time.sleep(0.5)
